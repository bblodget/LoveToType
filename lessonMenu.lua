	...
	for n,b in pairs(self.button) do
		if b:mousepressed(x,y,button) then
			...
			elseif n == "four" then
				...
				self.button.sub2 = Button.createTextButton("Lesson 4 Review",cx,yy+(s*1))
...
function LessonMenu:sub1()
	...
	elseif (self.level == 3) then
		lesson = Lesson.create(data_3a)
		state = lesson:next()
	elseif (self.level == 4) then
		lesson = Lesson.create(data_4a)
		state = lesson:next()
	end
end
...
function LessonMenu:sub2()
	if (self.level == 1) then
		...
		state = lesson:next()
	elseif (self.level == 2) then
		lesson = Lesson.create(data_2b)
		state = lesson:next()
	elseif (self.level == 3) then
		lesson = Lesson.create(data_3b)
		state = lesson:next()
	elseif (self.level == 4) then
		lesson = Lesson.create(data_4r,9) -- step 9 for review level
		state = lesson:next()
	end
end
...
function LessonMenu:sub3()
	...
	elseif (self.level == 2) then
		...
		state = lesson:next()
	elseif (self.level == 3) then
		lesson = Lesson.create(data_3r,9) -- step 9 for review level
		state = lesson:next()
	end
end
